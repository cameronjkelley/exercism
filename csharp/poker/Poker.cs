...
public static class Poker
{
    ...
    private static int ScoreHand(Ranks rank, List<Card> cards)
    {
        ...
        switch (rank)
        {
            ...
            case Ranks.ThreeOfAKind:
                ...
                break;
            case Ranks.TwoPair:
                score = cards.GroupBy(c => c.Value).Where(g => g.Count() == 2).Sum(g => (int)Math.Pow(g.Key, 2)) + Kicker(cards);
                break;
            case Ranks.Pair:
                score = cards.GroupBy(c => c.Value).FirstOrDefault(g => g.Count() == 2).Key + Kicker(cards);
                break;
            default:
    ...
    private static int Kicker(List<Card> cards)
    {
